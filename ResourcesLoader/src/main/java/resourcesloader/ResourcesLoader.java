...
import communication.ServerInfo;
...
public class ResourcesLoader {
    ...
    private void createServers(int initialPort, int numberOfServers){
        ...
        try {
            ...
            for(int i = 0; i < numberOfServers; i++){
                ...
                //Stores public key
                ServerInfo serverInf = new ServerInfo(address, initialPort + i, keyPair.getPublic());
            ...
            if(userNotary){
                ...
                ServerInfo serverInf = new ServerInfo(address, notaryPort, notaryPublicKey);
                servers.add(serverInf);
            }
     ...
     */
    public static void storeServers(List<ServerInfo> servers) throws IOException {
     ...
     */
    public static List<ServerInfo> loadServersInfo() throws IOException, ClassNotFoundException {
        // Deserialize
        ObjectInputStream in = new ObjectInputStream(new FileInputStream(resourcesPath+"serverInfo.ser"));
        List<ServerInfo> users = (List<ServerInfo>) in.readObject();
        in.close();
